...
import { Swords, Trophy, Clock, CheckCircle2, User } from 'lucide-react';
...
const ccsResults: GameItem[] = [
  { id: '1', sport: 'Basketball (M)', matchup: 'CCS vs CAIS', timeOrScore: '84 - 72 (Won)', isWin: true, credit: 'James Abubakar' },
  { id: '2', sport: 'Sepak Takraw', matchup: 'CCS vs CCJE', timeOrScore: '2 - 0 (Won)', isWin: true, credit: 'Althea Ramos' },
  { id: '3', sport: 'Table Tennis', matchup: 'CCS vs CN', timeOrScore: '1 - 3 (Lost)', isWin: false, credit: 'Mark Reyes' },
  { id: '4', sport: 'MLBB Esports', matchup: 'CCS vs CBA', timeOrScore: '2 - 0 (Won)', isWin: true, credit: 'Princess Tan' },
];
...
export const GameScheduleCard: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#c5d8c3] p-4 sm:p-5 shadow-xs flex flex-col h-full">
      {/* Toggle Controls */}
      <div className="flex items-center justify-between pb-3 mb-3 border-b border-[#e2ece0] gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <div className="w-7 h-7 rounded-lg bg-[#edf5ec] flex items-center justify-center border border-[#c5d8c3]/60 shrink-0">
            {activeTab === 'upcoming' ? (
          ...
          {/* Header Title */}
          <span className="text-xs sm:text-sm font-bold text-[#1f381f] uppercase tracking-wide font-display truncate">
            <span className="sm:hidden">CCS Games</span>
            <span className="hidden sm:inline">College of Computing Studies Games</span>
          </span>
        </div>

        {/* Tab switch */}
        <div className="flex bg-[#edf5ec] p-1 rounded-xl border border-[#c5d8c3]/60 text-xs shrink-0">
          <button
            type="button"
      ...
      {/* Content List */}
      <div className="space-y-2.5 overflow-y-auto max-h-[300px] sm:max-h-[330px] pr-1">
        {list.map((item) => (
          <div
            key={item.id}
          ...
          >
            ...
            <div className="flex items-center justify-between text-[11px] text-stone-500 font-mono pt-0.5">
              ...
              {/* Individual Per-Game Credit Badge with Person Icon */}
              {activeTab === 'results' && item.credit && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-white border border-[#c5d8c3]/80 text-[10.5px] font-medium text-stone-700 font-sans shadow-2xs">
                  <User className="w-3 h-3 text-[#355935]" />
