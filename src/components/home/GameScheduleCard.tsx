...
import { Clock, CheckCircle2, ChevronRight, X, MapPin, Users, UserRound, ShieldCheck, Trophy, Radio } from 'lucide-react';
      ...
      <div className="px-4 sm:px-5 py-3 sm:py-3.5 flex items-center justify-between gap-3 min-h-[64px]">
        ...
        <div className="flex items-center bg-[#f1f5f0] rounded-xl p-0.5 border border-[#dae8d7] shrink-0 text-[11px] font-mono font-semibold">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg transition-all cursor-pointer ${activeTab === 'upcoming'
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg transition-all cursor-pointer ${activeTab === 'results'
                ? 'bg-[#355935] text-white shadow-xs'
                : 'text-stone-500 hover:text-[#355935]'
              }`}
          >
      ...
      <div className="flex-1 flex flex-col divide-y divide-[#eef5ed]">
        {list.map((item) => (
          ...
          >
            {/* Left: accent bar */}
            <div
              className={`w-1 self-stretch rounded-full shrink-0 ${activeTab === 'results'
            ...
            <div className="shrink-0 flex flex-col items-end gap-1 text-right font-mono">
              {activeTab === 'upcoming' ? (
                ...
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white border border-[#c5d8c3] font-semibold text-[11px] sm:text-xs shadow-2xs">
                  <CheckCircle2
                    className={`w-3.5 h-3.5 shrink-0 ${item.isWin ? 'text-emerald-600' : 'text-rose-600'
                      }`}
                  />
                  ...
                  <span
                    className={`ml-0.5 text-[9px] font-bold uppercase px-1.5 py-0.2 rounded-full ${item.isWin
          ...
          >
            ...
            <div className="bg-[#142614] text-white p-3.5 sm:p-4 border-b border-[#355935] flex items-center justify-between shrink-0">
              <div>
                ...
                <h3 id="game-info-title" className="text-sm sm:text-base font-bold font-display uppercase tracking-wide mt-0.5 text-white">
            ...
            <div className="p-4 sm:p-5 space-y-3 sm:space-y-4 flex-1 overflow-y-auto">
              {/* Match Details Pill (BackupRequest style) */}
              <div className="bg-[#f6f9f5] border border-[#d6e5d5] rounded-xl p-2.5 sm:p-3 text-xs font-mono">
                <div className="flex items-center justify-between">
                  <span className="text-[#355935] font-bold uppercase">{selectedGame.sport}</span>
                  <span className={`inline-flex rounded-md border px-1.5 py-0.5 text-[9px] font-bold uppercase ${activeTab === 'upcoming'
                      ? 'border-[#c5d8c3] bg-[#edf5ec] text-[#355935]'
                      : selectedGame.isWin
                        ? 'border-emerald-300 bg-emerald-100 text-emerald-800'
                        : 'border-rose-300 bg-rose-100 text-rose-800'
                    }`}>
                    {activeTab === 'upcoming' ? 'UPCOMING' : selectedGame.isWin ? 'VICTORY' : 'DEFEAT'}
                  </span>
                </div>
