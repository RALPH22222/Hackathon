        ...
        <div className="flex-1 flex flex-col divide-y divide-[#eef5ed]">
          {facilitatorHandledMatches.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`flex-1 flex items-center gap-3 px-4 sm:px-5 py-3 hover:bg-[#f8faf7] transition-colors ${isLive ? 'bg-amber-50/50' : ''
                  }`}
              >
                {/* Left status bar */}
                <div
                  className={`w-1 self-stretch rounded-full shrink-0 ${isLive ? 'bg-amber-500' : isCompleted ? 'bg-emerald-500' : 'bg-[#c5d8c3]'
                    }`}
                />
                ...
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-[12px] sm:text-[13px] font-bold leading-snug truncate font-display ${isLive ? 'text-amber-950' : 'text-[#1a2f1a]'
                      }`}
                  >
                    {item.sport}
                  </p>
                ...
                <div className="shrink-0 text-right font-mono">
                  <div
                    className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white border font-semibold text-[11px] sm:text-xs shadow-2xs ${isLive ? 'border-amber-300' : 'border-[#c5d8c3]'
                      }`}
                  >
                    <Clock
                      className={`w-3.5 h-3.5 shrink-0 ${isLive ? 'text-amber-600' : 'text-[#355935]'
                        }`}
                    />
                    ...
                    <span
                      className={`ml-0.5 text-[9px] font-bold uppercase px-1.5 py-0.2 rounded-full ${isLive
        ...
        <div className="flex-1 flex flex-col divide-y divide-[#eef5ed]">
          {adviserData.events.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`flex-1 flex items-center gap-3 px-4 sm:px-5 py-2.5 sm:py-3 transition-colors ${isLive ? 'bg-amber-50/40' : 'hover:bg-[#f8faf7]'
                  }`}
              >
                {/* Status Bar */}
                <div
                  className={`w-1 self-stretch rounded-full shrink-0 ${isLive ? 'bg-amber-500' : 'bg-[#c5d8c3]'
                    }`}
                />
                ...
                <div className="shrink-0 text-right font-mono">
                  <div
                    className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-lg bg-white border font-semibold text-[10.5px] sm:text-xs shadow-2xs ${isLive ? 'border-amber-300' : 'border-[#c5d8c3]'
                      }`}
                  >
                    <Clock
                      className={`w-3 h-3 shrink-0 ${isLive ? 'text-amber-600' : 'text-[#355935]'
                        }`}
                    />
                    <span className={isLive ? 'text-amber-950 font-bold' : 'text-[#1f381f]'}>
                      {item.time}
                    ...
                    <span
                      className={`ml-0.5 text-[8.5px] font-bold uppercase px-1.5 py-0.2 rounded-full ${isLive
                          ? 'bg-amber-100 text-amber-800'
                          : 'bg-[#edf5ec] text-[#355935]'
                        }`}
                    >
        ...
        {sectionSchedules.map((item) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p
                  className={`text-[12px] sm:text-[13px] font-bold leading-snug truncate font-display ${isLive ? 'text-amber-900' : 'text-[#1a2f1a]'
                    }`}
                >
                  {item.sport}
                </p>
            ...
            <div className="p-4 sm:p-5 space-y-3 sm:space-y-4 flex-1 overflow-y-auto">
              ...
              <div className="bg-[#f6f9f5] border border-[#d6e5d5] rounded-xl p-2.5 sm:p-3 text-xs font-mono">
                <div className="flex items-center justify-between">
                  <span className="text-[#355935] font-bold uppercase">{selectedAttendance.sport}</span>
                  <span className="inline-flex rounded-md border border-[#c5d8c3] bg-[#edf5ec] px-1.5 py-0.5 text-[9px] font-bold uppercase text-[#355935]">
                    PALARO 2026
                  </span>
                </div>
                <div className="text-stone-700 mt-1 flex items-center gap-1.5">
                  <MapPin className="w-3.5 h-3.5 text-[#5d8c55] shrink-0" />
                  <span className="truncate">{selectedAttendance.venue}</span>
                </div>
                <div className="text-[10px] sm:text-[11px] text-stone-500 mt-0.5 flex items-center gap-1.5">
                  ...
                  <span>Schedule: {selectedAttendance.startTime} - {selectedAttendance.endTime}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 text-xs font-mono">
                <div className="rounded-xl bg-[#f8faf8] border border-[#e5efe4] p-2.5 sm:p-3 space-y-1">
                  ...
                  <p className="font-semibold text-[#1f381f] text-xs leading-snug">
                    {selectedAttendance.facilitator.replace('Facilitator: ', '')}
                  </p>
                  <span className="inline-block rounded-md border border-amber-300 bg-amber-100 px-1.5 py-0.2 text-[9px] font-bold uppercase text-amber-800">
                    PROF · {selectedAttendance.facilitatorProgram}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <h4 className="text-[11px] font-mono font-bold uppercase tracking-wider text-stone-700">
                      Rostered Athletes ({selectedAttendance.players.length})
                    </h4>
                  </div>
