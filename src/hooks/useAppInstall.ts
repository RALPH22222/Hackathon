...
export function useAppInstall() {
  ...
  useEffect(() => {
    ...
    const handleAppInstalled = () => {
      ...
      console.log('[App] Application was successfully added to home screen.');
    };
  ...
  const promptInstall = async () => {
    ...
    try {
      ...
      if (choiceResult.outcome === 'accepted') {
        console.log('[App] User accepted app installation');
        setIsInstalled(true);
      }
      setDeferredPrompt(null);
      return choiceResult.outcome === 'accepted';
    } catch (err) {
      console.error('[App] Error triggering app installation prompt:', err);
      return false;
    }
  };
